...
void CNestApi::GetMeterDetails()
{
	...
	for (Json::Value::iterator itThermostat = root["devices"]["thermostats"].begin(); itThermostat != root["devices"]["thermostats"].end(); ++itThermostat)
	{
		...
		_log.Log(LOG_NORM,("***** CNestApi::GetMeterDetails - TARGET TEMPERATURE  = "+boost::to_string(targetTemp)).c_str());
		...
		_log.Log(LOG_NORM,("***** CNestApi::GetMeterDetails - AMBIENT TEMPERATURE = "+boost::to_string(ambientTemp)).c_str());
